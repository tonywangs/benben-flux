...
Authorize the **jordan-tony-collab** workspace in the browser. This project uses that workspace’s credits and the `main` environment. The explicit profile flags below leave other projects’ default workspace unchanged.
...
3. In [Modal Secrets](https://modal.com/secrets), create `huggingface-secret` containing `HF_TOKEN`. Select workspace **jordan-tony-collab** and environment **main** for both secrets. Keep tokens out of this repository and chat.
...
```sh
uv run modal run --profile jordan-tony-collab --env main --detach modal_app.py::train --photos ./photos --run-id benben-v1
```
...
```sh
uv run modal run --profile jordan-tony-collab --env main --detach modal_app.py::train --run-id benben-v1 --resume
```
...
```sh
uv run modal run --profile jordan-tony-collab --env main --detach modal_app.py::train --run-id benben-v2 --steps 800 --rank 16
```

## Generate and share

```sh
uv run modal run --profile jordan-tony-collab --env main modal_app.py::generate --run-id benben-v1 \
...
uv run modal serve --profile jordan-tony-collab --env main modal_app.py
...
```sh
uv run modal volume get --profile jordan-tony-collab --env main benben-flux /runs/benben-v1/lora/pytorch_lora_weights.safetensors ./outputs/
```
